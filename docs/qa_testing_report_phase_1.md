...
*   **用例 A: 音频病理穿透断言 (`test_stt_standard_audio_handling`)**
    ...
    *   **断言逻辑 (Asserts)**: 
        ...
        2. 深层断言引擎是否成功在噪音/口音中捕获了关键词 `patient` 及 `abdominal pain` (腹痛)。
        3. 强制校验 `initial_prompt` 的约束效果：输出文本必须以大写字母开头，结尾带合法标点 (`.`, `?`, `!`)。
    *   **结果**: **[PASS]**

*   **用例 B: OOM 内核绞杀与主进程存活测试 (`test_stt_oom_isolation_survival`)**
    *   **测试桩 (Fixture)**: 采用 `multiprocessing.get_context('spawn')` 物理隔离空间；通过注入 `malicious_oom_worker` 发出恶意的 `SIGKILL` 自尽指令。
    *   **断言逻辑 (Asserts)**: 子进程非正常死亡时，主控循环必须捕获 `p.exitcode != 0` 的死亡标识。断言必须抛出且仅抛出被包裹的 `MemoryError("STT Process killed violently")` 异常，而绝不波及外层 Pytest 主线程。
    *   **结果**: **[PASS]**
...
*   **用例 C: 绝对人名销毁 (`test_privacy_filter_person_redaction`)**
    ...
    *   **断言逻辑**: 全句经过 `ClinicalPrivacyFilter.mask_pii()` 后，断言文本内绝对不包含字符串 "Emily", "Chen", "James", "Wilson"。且替换符 `[REDACTED]` 必须合法占位成功。
    *   **结果**: **[PASS]**

*   **用例 D: 敏感数字与证件封堵 (`test_privacy_filter_ssn_and_phone`)**
    *   **测试桩**: 带有美国社保号 (012-34-5678) 和常规电话号 (555-019-8372) 的文案。
    *   **断言逻辑**: 数字串全部检索不到。断言含有至少两次 `[REDACTED]` 屏蔽。
    *   **结果**: **[PASS]**

*   **用例 E: 空载抗性 (`test_privacy_filter_empty_string`)**
    *   **测试桩**: 纯空格或 `""` 字符串传输。
    *   **断言逻辑**: 引擎不触发崩溃或进行无效正则扫描，直接返回空格本身。
    *   **结果**: **[PASS]**

### 2.3 LLM 解析适配器 (`test_deepseek_adapter.py`)
此环节在不消耗 API Key 真实 Token 的情况下验证基于 OpenAI SDK 的接口解析容错率。

*   **用例 F: Strict-JSON 强路由格式解析 (`test_deepseek_adapter_json_structure`)**
    *   **测试桩**: 使用 `unittest.mock.patch` 魔改拦截底层的发送长链接，向系统瞬速塞回一段人工编写好的 DeepSeek 伪造大模型回复 (一段嵌套了 dialogue 和 soap 的纯 JSON Text)。
    *   **断言逻辑**: 验证提取层能否不抛异常地使用 `json.loads` 加载，同时探针强锁底层的四级字典树结构：`soap` 下必须含有 `subjective`, `objective`, `assessment`, `plan`。
    *   **结果**: **[PASS]**

*   **用例 G: 空集熔断保护 (`test_deepseek_adapter_empty_transcript`)**
    *   **测试桩**: 输入空白的 STT 稿件。
    *   **断言逻辑**: 网络发包层必须直接报出 `ValueError("Transcript cannot be empty.")` 提前截断，保护钱包余额。
    *   **结果**: **[PASS]**
