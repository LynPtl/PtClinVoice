...
## 1. 基础设施健康度检查 (Infrastructure Health Checks)
...
```
> **故障排查 (Troubleshooting)**: 如果长时间无响应请求超时，极大概率是因云接入商网络安全组限制或宿主机内部的 `iptables` 未放行入站 `8000` 端口。如果返回 `Connection Refused`，请登录宿主机执行 `docker ps -a`，排查容器是否由于环境变量 `.env` 缺失或镜像构建阶段异常导致未能成功持久运行。
...
http://<宿主机_IP>
```
* **[PASS] [预期行为]**: 浏览器立即重定向至 `http://<宿主机_IP>/login`，呈现带有 "Welcome to PtClinVoice" 和医疗配色的登录表单，而不是浏览器默认的 404 或 Nginx 欢迎页。
...
2. **点击提交**: 
   - **[PASS] [成功预期]**: 控制台将跳转回 `/dashboard`，界面上渲染出对应的欢迎词。这同时也**侧面证明了**部署在 Nginx 端内的 `location /api/ { proxy_pass http://ptclinvoice-api:8000; }` 内部反向解析链路100% 完美连通且规避了同源 CORS 跨域。
   - **[FAIL] [失败情形]**: 若浏览器 DevTools 的 Network 面板抛出针对 `/api/auth/login` 的 `502 Bad Gateway` 报错，表明 Nginx 网关已就绪，但后方的 FastAPI 服务容器进程异常中止或未挂载（需排查 `ptclinvoice-api` 容器日志）。

### 2.2 上发通道与物理防泄漏测试 (Upload & Shredding Pipeline)
1. 在 Dashboard 核心区域，上传目标 `.wav` 或 `.mp3` 音频文件。
2. 上传完毕后立刻进入右侧的 **历史任务列表 (Task List)**。
3. **[PASS] [动态同步预期]**: 您应该看到该任务以 `PENDING` 态展示。关键验证点：**不要手动刷新页面**，几十秒内它必须自动演变成 `COMPLETED`（代表基于 SSE 技术的服务端长连接推送通道已穿透 Nginx）。
4. **[PASS] [隐私销毁预期 (Burn-after-reading)]**: 此时，SSH 切入云主机，执行内部探查：
    ```bash
    # 进入宿主机数据持久化挂载目录
...
```
**[PASS] [预期结果]**: API 拒绝率为 0%，并且宿主机的 SQLite 持久化目录 `~/ptclinvoice_deploy/data` 内的 `.db-wal` 的尺寸发生剧烈抖动然后快速合并，说明存储引擎运作在最优状态。
