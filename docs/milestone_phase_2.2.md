...
| **编排层健康自愈** | `docker-compose.yml` | 引入基于 Python 内置库的极简原生 Healthcheck 机制 (`urllib.request`)，每 30 秒探测 FastAPI `/health` 端点。在云端仅开放外侧端口、可能存在反代阻断或工作流假死时，赋予 Docker 引擎自动诊断与执行 `restart` 的终末兜底能力。 |
