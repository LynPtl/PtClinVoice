...
### 1. 生产环境镜像部署 (Containerized Execution)
推荐使用官方托管于 GitHub Container Registry (GHCR) 的预编译镜像。镜像内已包含 `ffmpeg` 解码加速引擎以及 SpaCy `en_core_web_sm` 自然语言模型底座。
...
如需在本地环境解锁满血 RTF (Real-Time Factor) 性能，需配置专有的深度学习链接库：
...
| **物理碎纸机** | `PYTHONPATH=. pytest tests/test_upload_and_shred.py` | 断言 OS 级的文件销毁机制 |
...
*   **系统稳定性测试报告**：`docs/qa_testing_report_phase_3.md`
