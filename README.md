...
### 1. 环境准备
本项目采用纯 Python 3.10+ 开发环境，具备跨计算架构的可移植性（原生支持在 ARM64 云主机、x86 裸机以及 WSL 环境内部署运行）。
...
**A. 纯黑盒离线测试 (零网络发包，拦截验证)**:
```bash
pytest -v
```
- **STT 引擎拦截测试 (`test_stt_core.py`)**：会测试模型能否从本地 MP3 中跑出带有标点病灶英语。还包含 OOM (`SIGKILL`) 进程免疫测试。
- **DeepSeek 结构验证 (`test_deepseek_adapter.py`)**：Mock 拦截测试，验证 JSON SOAP 组装格式。
